...
* 5.5 meters times 4 meters with a playing area of 4.5 meters times 3 meters

The exact field dimensions and the field markings at the venue may vary by up to ±10% in each linear dimension.

The figure below show the dimensions of the field, the goals and special field areas, measured in millimeters between the line centers.

NOTE: The numbers in the figures below show the distances in millimeters between the line centers.

[[field-dimensions]]
.Field dimensions and markings for SSL-EL
...
===== Defense Area
A defense area is defined as a rectangle touching the goal lines centrally in front of both <<Goals, goals>>. The size of the defense area is 1.35 meters times 0.5 meters, as shown in figure <<field-dimensions, 1>>.

===== Penalty Mark
The penalty mark defines the point from which a team executes a penalty kick against the opponent goal. It is located on the <<Goal-to-Goal Line, goal-to-goal line>> and 3 meters away from the opponent's goal center.
...
The distance between the side walls is 0.8 meters, and the goal is 0.18 meters deep. The goal walls are 0.02 meters thick and touch the goal line, but do not overlap or encroach on the field lines or the field. <<goal-detail, Figure 2>> show these details.

NOTE: The numbers in the figures below show the distances in millimeters.

[[goal-detail]]
.The goal in detail for SSL-EL
image::goal_detail.svg[width=400]
...
=== Shared Software
The shared software used in the SSL-EL is the same as that used in the other standard categories, and it is maintained by the same <<Technical Committee, technical committee>>, though everyone is encouraged to contribute. The <<Technical Committee, technical committee>> members however guarantee that any changes made less than three months before the next RoboCup do not break compatibility.
